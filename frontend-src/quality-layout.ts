interface Window { ExpressionLayout: { enhance(stage: HTMLElement): void }; }

(function installLayout(root: Window) {
  ...
  root.ExpressionLayout = { enhance(stage) {
    ...
    Object.entries(groups).forEach(([selector, labels]) => {
      const layout = stage.querySelector<HTMLElement>(selector);
      if (!layout) return;
      const panels = Array.from(layout.children).filter((node): node is HTMLElement => node instanceof HTMLElement);
      layout.classList.add('switchable-layout');
      const label = document.createElement('label'); label.className = 'panel-picker';
      const text = document.createElement('span'); text.textContent = '查看';
      const select = document.createElement('select'); select.setAttribute('aria-label', '切换当前步骤的内容');
      panels.forEach((panel, index) => {
        panel.classList.add('switchable-panel');
        panel.dataset.panelIndex = String(index);
        const option = document.createElement('option'); option.value = String(index); option.textContent = labels[index] || `内容 ${index + 1}`; select.append(option);
      });
      ...
      if (layout.querySelector('#exam-answer')) select.value = '1';
      if (selector === '.ai-coach-panel') select.value = '1';
      select.addEventListener('change', activate); activate();
      label.append(text, select); layout.before(label);
    });
  } };
})(window);
